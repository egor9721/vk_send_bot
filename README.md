...
Ссылка для скачивания .exe файла под windows находится в каталоге dist/vk_bot.exe.
Чтобы его загрузить, необходимо открыт данный файл и нажать ctrl+shift+s.
Для применения использования нового .exe файла, необходимо заменить им старый файл и запустить.

# Изменения:

* Добавлена запись лога по работе приложения (пишутся в папку, где лежит файл с сообщением)
* Переписан интерфейс под Qt5
* добавлена поддержка всех видов ссылок на аккаунт пользователя включая vk.ru
...
# Информация по сборке билда под разные системы

1. необходимо установить все необходимые зависимости (разработка ведется на python 3.12, поэтому он должен быть установлен)
    `pip install -r requirements.txt`
2. запустить сборку билда командой:
     `pyinstaller --onefile --noconsole .\source\main.py`
3. по окончании сборки билд будет находится в каталоге `dist`
